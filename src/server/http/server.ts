...
function defaultAuthorizationErrorDescription(provider: ProviderKind, error: string): string {
...
export class EmailConnectHttpServer {
    ...
    const result =
      decision === 'approve'
        ...
        : this.engine.connect.denyAuthorizationRequest(
            requestId,
            'access_denied',
            defaultAuthorizationErrorDescription(provider, 'access_denied'),
          );
    res.statusCode = 302;
  ...
  private async handleGoogleConnect(
    ...
    if (method === 'POST' && pathname === '/token') {
      ...
      if (grantType === 'authorization_code') {
        await this.respondWithTokenGrant(
          res,
          () =>
            this.engine.connect.exchangeAuthorizationCode({
              provider: 'gmail',
      ...
      if (grantType === 'refresh_token') {
        await this.respondWithTokenGrant(
          res,
          () =>
            this.engine.connect.refreshAccessToken({
              provider: 'gmail',
              clientId: String(body.client_id || ''),
              ...(body.client_secret != null ? { clientSecret: body.client_secret } : {}),
  ...
  private async handleMicrosoftConnect(
    method: string,
    url: URL,
    req: IncomingMessage,
    res: ServerResponse,
  ...
  private async handleGmail(
    ...
    if (method === 'GET' && pathname === '/gmail/v1/users/me/messages') {
      ...
      sendJson(
        res,
        200,
        await this.gmail.listMessages(mailbox.id, {
    ...
    if (method === 'GET' && attachmentMatch?.[1] && attachmentMatch?.[2]) {
      ...
      sendJson(
        res,
        200,
        await this.gmail.getAttachment(mailbox.id, decodeURIComponent(attachmentMatch[1]), decodeURIComponent(attachmentMatch[2])),
      );
      return;
    }
    ...
    if (method === 'GET' && pathname === '/gmail/v1/users/me/history') {
      ...
      sendJson(
        res,
        200,
        await this.gmail.listHistory(mailbox.id, {
  ...
  private async handleGraph(
    ...
    if (method === 'GET' && attachmentsListMatch?.[1]) {
      ...
      sendJson(
        res,
        200,
        await this.graph.listAttachmentsPage(
          mailbox.id,
    ...
    const attachmentValueMatch = matchPath(
      pathname,
      /^\/graph\/v1\.0\/me\/messages\/([^/]+)\/attachments\/([^/]+)\/\$value$/,
    );
    if (method === 'GET' && attachmentValueMatch?.[1] && attachmentValueMatch?.[2]) {
      ...
      sendBytes(
        res,
        200,
        await this.graph.getAttachmentValue(
          mailbox.id,
          decodeURIComponent(attachmentValueMatch[1]),
          decodeURIComponent(attachmentValueMatch[2]),
        ),
      );
      return;
    }
    ...
    if (method === 'GET' && attachmentMatch?.[1] && attachmentMatch?.[2]) {
      ...
      sendJson(
        res,
        200,
        await this.graph.getAttachment(mailbox.id, decodeURIComponent(attachmentMatch[1]), decodeURIComponent(attachmentMatch[2])),
      );
      return;
    }
