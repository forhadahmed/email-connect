...
function normalizeScopeSet(scopes) {
    const out = [];
    const seen = new Set();
...
export class EmailConnectConnectPlane {
    ...
    seedMailboxGrant(mailboxId, seed) {
        ...
        const grantedScopes = normalizeScopeSet(seed?.scopes || this.engine.requireProvider(mailbox.provider).connect.defaultScopesForCapabilityMode(capabilityMode));
