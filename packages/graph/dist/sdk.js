import { decodeBase64ToBytes } from '@email-connect/core';
import { GraphService } from './service.js';
export function getOutlookGraphClientForMailbox(engine, mailboxId) {
    const service = new GraphService(engine);
    const normalizePath = (path) => {
        const stripGraphPrefix = (value) => value.startsWith('/graph/v1.0') ? value.slice('/graph/v1.0'.length) || '/' : value;
    ...
    const requestFactory = (rawPath) => {
        ...
        const request = {
            ...
            async get() {
                if (pathname === '/me')
                    return service.getMe(mailboxId);
                ...
                if (messageMatch && !pathname.includes('/attachments/')) {
                    ...
                    if (!messageId)
                        throw new Error(`Unsupported Graph message path: ${path}`);
                ...
                if (attachmentsMatch) {
                    ...
                    if (!messageId)
                        throw new Error(`Unsupported Graph attachments path: ${path}`);
                ...
                if (attachmentValueMatch) {
                    ...
                    if (!messageId || !attachmentId)
                        throw new Error(`Unsupported Graph attachment value path: ${path}`);
                    return service.getAttachmentValue(mailboxId, decodeURIComponent(messageId), decodeURIComponent(attachmentId));
                }
                ...
                if (attachmentMatch) {
                    ...
                    if (!messageId || !attachmentId)
                        throw new Error(`Unsupported Graph attachment path: ${path}`);
                    return service.getAttachment(mailboxId, decodeURIComponent(messageId), decodeURIComponent(attachmentId));
                }
            ...
            async post(body) {
                ...
                if (createReplyMatch) {
                    ...
                    if (!messageId)
                        throw new Error(`Unsupported Graph createReply path: ${path}`);
                ...
                if (sendMatch) {
                    ...
                    if (!draftId)
                        throw new Error(`Unsupported Graph send path: ${path}`);
                    return service.sendDraft(mailboxId, decodeURIComponent(draftId));
                }
                if (pathname === '/me/messages') {
                    return service.createDraft(mailboxId, (body || {}));
                }
            ...
            async patch(body) {
                ...
                if (!draftId)
                    throw new Error(`Unsupported Graph draft path: ${path}`);
