import { NotFoundError, encodeBytesBase64 } from '@email-connect/core';
function parseUrl(path, origin = 'https://graph.microsoft.com/v1.0') {
    return new URL(path, origin);
}
function parseTop(url, fallback) {
...
function receivedAfterFromFilter(value) {
    if (!value)
        return null;
...
function encodeDeltaUrl(baseUrl, path, params) {
...
function graphBodyFromPayload(payload) {
...
function draftPatchFromBody(body) {
...
function changeToDeltaItem(change, message) {
...
export class GraphService {
    ...
    async listMessages(mailboxId, path, baseUrl) {
        const mailbox = this.engine.requireMailbox(mailboxId);
        await this.engine.maybeDelay(mailbox);
        ...
        return {
            ...
            '@odata.nextLink': rows.length > offset + page.length
    ...
    async listInboxMessages(mailboxId, path, baseUrl) {
        const mailbox = this.engine.requireMailbox(mailboxId);
        await this.engine.maybeDelay(mailbox);
    ...
    async delta(mailboxId, path, baseUrl) {
        const mailbox = this.engine.requireMailbox(mailboxId);
        await this.engine.maybeDelay(mailbox);
        ...
        return {
            ...
            '@odata.nextLink': changes.length > offset + page.length
                ...
                : undefined,
            '@odata.deltaLink': changes.length > offset + page.length
                ? undefined
                : encodeDeltaUrl(baseUrl, '/graph/v1.0/me/mailFolders/inbox/messages/delta', {
    ...
    async listAttachments(mailboxId, providerMessageId) {
        const mailbox = this.engine.requireMailbox(mailboxId);
        await this.engine.maybeDelay(mailbox);
    ...
    async listAttachmentsPage(mailboxId, providerMessageId, path, baseUrl) {
        const mailbox = this.engine.requireMailbox(mailboxId);
        await this.engine.maybeDelay(mailbox);
        this.engine.maybeThrowInjectedFailure(mailbox, 'graph.attachments.list');
        const message = this.engine.listVisibleMessages(mailbox).find((entry) => entry.providerMessageId === providerMessageId);
        ...
        return {
            ...
            '@odata.nextLink': message.attachments.length > offset + page.length
    ...
    async getAttachmentValue(mailboxId, providerMessageId, attachmentId) {
        const mailbox = this.engine.requireMailbox(mailboxId);
        await this.engine.maybeDelay(mailbox);
    ...
    async createReplyDraft(mailboxId, providerMessageId) {
        const mailbox = this.engine.requireMailbox(mailboxId);
        await this.engine.maybeDelay(mailbox);
    ...
    async createDraft(mailboxId, body) {
        const mailbox = this.engine.requireMailbox(mailboxId);
        await this.engine.maybeDelay(mailbox);
    ...
    async patchDraft(mailboxId, providerDraftId, body) {
        const mailbox = this.engine.requireMailbox(mailboxId);
        await this.engine.maybeDelay(mailbox);
    ...
    async sendDraft(mailboxId, providerDraftId) {
        const mailbox = this.engine.requireMailbox(mailboxId);
        await this.engine.maybeDelay(mailbox);
    ...
    async deleteMessageResource(mailboxId, providerMessageId) {
        const mailbox = this.engine.requireMailbox(mailboxId);
        await this.engine.maybeDelay(mailbox);
